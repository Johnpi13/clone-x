...
const listTweets = async (request, response) => {
  try {
    ...
    const tweets = await Tweet.find({ userName: userName }).sort({
      createdAt: -1,
    });
    ...
    response
      .status(500)
      .json({ message: "Error al obtener los tweets", error: err });
  }
};
...
const updateTweet = (request, response) => {
  ...
  if (tweetIndex !== -1) {
    ...
    response
      .status(200)
      .json({ msg: "Tweet actualizado", tweet: tweets[tweetIndex] });
  } else {
